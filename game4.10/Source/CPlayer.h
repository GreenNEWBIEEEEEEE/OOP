...
namespace game_framework {
	...
	public:
		...
		bool DetectLeftElementID(CGameMap* m, vector<int> elemID, int distance = 1);   // 用來偵測玩家目前方向是否為要偵測的地圖elemID
		bool DetectRightElementID(CGameMap* m, vector<int> elemID, int distance = 1);  // 第一參數：玩家目前地圖，第二參數：要偵測的地圖elemID，第三參數：偵測的範圍
